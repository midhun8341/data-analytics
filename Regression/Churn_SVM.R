...
library(e1071)
model <- svm(Churn. ~ . , Train_data, cost = 10 , kernel = 'linear')
model
plot(model$fitted, Train_data$Churn.)
...
model.1 <- svm(Churn. ~ CustServ.Calls + Intl.Calls + VMail.Plan + Int.l.Plan, Train_data, cost = 10 , kernel = 'linear')
model.1
plot(model.1$fitted, Train_data$Churn.)
...
library(caret)
confusionMatrix(pred.1, Test_data$Churn.)
#Accuracy : 0.837
