...
#7.Developing the model <- Random_Forests
names(churn)
model.1 <- glm(Churn. ~ . , Train_data, family = "binomial")
model.1
summary(model.1)
pred <- predict(model.1, Test_data, type = "response")
pred
...
model.2 <- glm(Churn. ~ CustServ.Calls + Intl.Calls + VMail.Plan + Int.l.Plan, Train_data, family = "binomial")
model.2
summary(model.1)
pred.2 <- predict(model.2, Test_data, type = "response")
pred.2
pred.3 <- ifelse(pred.2 <= .5, 0, 1)
table(pred.3, Test_data$Churn.)
library(Metrics)
accuracy(pred.3, Test_data$Churn.)
#0.87

library(caret)
confusionMatrix(pred.3, Test_data$Churn.)
